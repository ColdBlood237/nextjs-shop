      ...
      .addCase(fetchProducts.fulfilled, (state, action) => {
        ...
        state.products = state.products.map(
          (product: { [key: string]: any }, index: number) => {
            product.price = productsInfo[index].price;
            product.description = productsInfo[index].description;
            product.image = productsInfo[index].image;
          }
        );
        ...
        // state.products[7].description = "An affordable option with great features.";
...
export const { sortByPriceAscending, sortByPriceDescending } = productsSlice.actions;
